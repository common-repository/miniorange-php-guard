...
class Scrambler
{
    ...
    function __construct($type,$conf,$target_directory)
    {
        ...
        if (isset($target_directory))                                   // the constructor will restore previous saved context if exists
        {
            ...
            if (file_exists("{$this->context_directory}/miniorange/context/{$this->scramble_type}"))
            {
                $t = unserialize(file_get_contents("{$this->context_directory}/miniorange/context/{$this->scramble_type}"));
                if ($t[0] !== self::SCRAMBLER_CONTEXT_VERSION)
                {
                    echo "Error:\tContext format has changed!";
    ...
    function __destruct()
    {
        ...
        if (!$this->silent) echo "Info:\t[$this->scramble_type] scrambled \t: ".count($this->t_scramble)."<br>";
        if (isset($this->context_directory))                            // the destructor will save the current context
        {
            ...
            file_put_contents("{$this->context_directory}/miniorange/context/{$this->scramble_type}",serialize($t));
        }
    }
    ...
    public function scramble($s)
    {
        ...
        if (!isset($this->t_scramble[$r]))      // if not already scrambled:
        {
            ...
            {
                echo "Scramble Error: Identifier not found after 20 iterations!<br>Aborting...<br>"; // should statistically never occur!
                exit;
            }
        }
